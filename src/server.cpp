...
bool Server::init() {
    ...
    // Set socket options
    if (setsockopt(this->server_socket, SOL_SOCKET, SO_REUSEADDR, &flags, sizeof(flags)) == -1) {
...
void Server::wait_for_socket_ready(const int client_socket) {
    ...
    switch (select(client_socket + 1, nullptr, &write_fds, nullptr, &this->timeout)) {
        case -1:
            Logger::LOG_ERROR("Error in select (send retry): " + std::string(strerror(errno)));
            break;
        case 0:
...
void Server::handle_new_connection() {
    ...
    fcntl(client_socket, F_SETFL, flags | O_NONBLOCK);
        ...
        constexpr int set = 1;
        if (setsockopt(client_socket, SOL_SOCKET, SO_NOSIGPIPE, &set, sizeof(set)) == -1) {
    ...
    Logger::LOG_INFO("New connection from " + std::string(inet_ntoa(client_address.sin_addr)));
}

void Server::handle_client(const int client_socket) {
    char in_buffer[1024];
    const long bytes_received = recv(client_socket, in_buffer, sizeof(in_buffer), 0);
    ...
    if (auto buffer = string(in_buffer, bytes_received); buffer.find("\r\n\r\n") != string::npos) {
        // Create request
...
void Server::handle_request(const Request& request, const int client_socket) {
    // Check keep-alive
    const bool keep_alive = request.headers.find(http::HTTPHeaders::CONNECTION)->second != "close";
    // Handle request
...
void Server::handle_get_request(const Request& request, const int client_socket, const bool keep_alive) {
    ...
    try {
        ...
        const bool use_cache = request.headers.find(http::HTTPHeaders::CACHE_CONTROL)->second != "no-cache";
...
ssize_t Server::send_with_retry(const int client_socket, const string& data) {
    ...
    do {
        ...
        if (errno == EAGAIN || errno == EWOULDBLOCK) {
            ...
            if (FD_ISSET(client_socket, &master) == 0) break;
        }
        else break;
    ...
    if (result <= 0) {
        if (errno != EPIPE)
            Logger::LOG_WARNING("Error sending data: " + std::string(strerror(errno)));
        close_session(client_socket);
    }
...
void Server::send_response(Response &response, const int client_socket, const bool keep_alive) {
    ...
    while (offset < body.size()) {
        ...
        const size_t chunk_size = min(Server::DEFAULT_BUFFER_SIZE, body.size() - offset);
